...
insert into products(product_id,product_name,product_description,product_category_id,product_hsn_code,product_type,product_reorder_level,Ecommerce_Reorder_Level,Product_Status)
select 01,'MI Laptop','Its a mini Computer',101,181818,'Regular',10,5,'Active' union all
select 02,'Realme Moblie','A Mobile  is a small, portable personal item with a screen',101,181818,'Regular',10,5,'Active'union all
select 03,'Apple Smart Watch','A smart watch display time and also used to operate some moblie opertions',101,181818,'Regular',10,5,'Active' union all
select 04,'Sony Camera','used to Click Good Pics',101,181818,'Regular',10,5,'Active' union all
select 11,'Hindware Kitchen','used in kitchen',202,202020,'Regular',10,5,'Active' union all
select 12,'Surya Lights','used to in dark to have some some sight',202,202020,'Regular',10,5,'Active' union all
select 13,'SleepX Beds','have a comfort sleep',202,202020,'Regular',10,5,'Active' union all
select 14,'Appolo Medicine','used when you are sick',202,202020,'Regular',10,5,'Active' union all
select 21,'Loreal Skin Care','need a glow use this',303,202020,'Regular',10,5,'Active' union all
select 22,'Loreal Hair Care','use and loose',303,202020,'Regular',10,5,'Active' union all
select 23,'Fogg Fragrances','smell the delight',303,202020,'Regular',10,5,'Active' union all
select 24,'Beardo Men Grooming','use the grooming andgetready to be the groom',303,202020,'Regular',10,5,'Active' union all
select 31,'Mi Television','Watch the Show and get chill',404,282828,'Regular',10,5,'Active' union all
select 32,' Comtron Fans','Feel the air',404,282828,'Regular',10,5,'Active' union all
select 33,'Loyyod Ac','Be Cool in the Cool',404,282828,'Regular',10,5,'Active' union all
select 34,'Samsung Refrigerators','keep it and get the Cooled out',404,282828,'Regular',10,5,'Active' union all
select 35,'Samsung Washing Machine',' keep cloths in and give rest to the hands',404,282828,'Regular',10,5,'Active' union all
select 41,'Cool cake','feel the bike',505,202020,'Regular',10,5,'Active'  union all
select 42,'Arun ice cream','take the cool in and be chilled',505,202020,'Regular',10,5,'Active' union all
select 43,'Maggie noodeles','get it ready in 1min',505,202020,'Regular',10,5,'Active' union all
select 44,'dal','try it with Mango pickle ',505,202020,'Regular',10,5,'Active' union all
select 45,'rice','get all the portions inn',505,202020,'Regular',10,5,'Active' union all
select 51,'Nike Sports Ware','Look Fit',606,505050,'Regular',10,5,'Active' union all
select 52,'Nike Sports Shoes','Select the sports and the shoe',606,505050,'Regular',10,5,'Active' union all
select 53,'Cricket Kit','Get the whole kit',606,505050,'Regular',10,5,'Active' union all
select 54,'Dumbels Fitness Equipments','Get the Kit and be fit ',606,505050,'Regular',10,5,'Active' union all
select 55,'Energo Supplements','Get the power and get fit',606,505050,'Regular',10,5,'Active' union all
select 61,'Allen Solly Men ware','Look Fit',707,121212,'Regular',10,5,'Active' union all
select 62,'Allen solly Women Ware','Select the sports and the shoe',707,121212,'Regular',10,5,'Active' union all
select 63,'Allen SollyKids ware','Get the whole kit',707,121212,'Regular',10,5,'Active';
...
Insert into Vendor ( 
	...
	select 20002,'Haritha','Vizag',9012345678,'Active',10003 union ALL
	select 20003,'Ritesh','Vijayawada',9012345678,'NotActive',10004 union All
	select 20004,'Mannu','Vizag',9012345678,'Active',10004 union ALL
...
--product id starts from 01

select 01,8123,10,50000,49000,49500 union all
select 02,8234,11,30000,29000,29500 union all
select 03,8345,12,150000,149000,149500 union all
select 04,8567,15,2000,1000,1500 union all
select 11,8456,18,5000,4000,4500 union all
select 12,8678,17,500,400,450 union all
select 13,8789,15,50000,49000,49500 union all
select 14,8147,15,5000,4800,4950 union all
select 21,8258,12,500,490,495 union all
select 22,8369,12,500,490,495 union all
select 23,8987,16,500,490,495 union all
select 24,8654,10,500,490,495 union all
select 31,8321,16,20000,19000,19500 union all
select 32,8159,18,5000,4900,4950 union all
select 33,8753,50,50000,49000,49500 union all
select 34,8842,50,30000,29000,29500 union all
select 35,8965,10,20000,19000,19500 union all
select 41,8842,20,800,750,790 union all
select 42,8571,30,500,400,490 union all
select 43,8862,50,50,40,45 union all
select 44,8196,25,80,70,78 union all
select 45,8367,24,50,40,49 union all
select 51,8958,21,50000,49000,49500 union all
select 52,8153,13,500,400,490 union all
select 53,8453,14,5000,4800,4900 union all
select 54,8756,19,10000,9000,9500 union all
select 55,8864,27,5000,4000,4500 union all
select 61,8456,12,500,400,450 union all
select 62,8457,20,500,400,450 union all
select 63,8458,10,500,400,450;


 Insert into eIndents ( 
...
select 9001,1,10 union all
select 9001,2,10 union all
select 9001,3,10 union all
select 9002,11,10 union all
select 9002,12,1 union all
select 9002,13,5 union all
select 9003,14,10 union all
select 9003,21,10 union all
select 9004,22,1;
...
SELECT 1, 1, 15, 85000, 50000, 75000 UNION all
SELECT 2, 23, 150, 25000, 10000, 23000 UNION all
...
SELECT 1, 1, 1500, 85000, 50000, 75000 UNION all
SELECT 2, 23, 500, 25000, 10000, 23000 UNION all
